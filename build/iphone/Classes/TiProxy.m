...
//Rare exceptions to indicate a bug in the _drinklistr code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_drinklistr = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_drinklistr]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.drinklistr.module"
